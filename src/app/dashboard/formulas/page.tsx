            ...
            <Card key={formula.id} className="shadow-md hover:shadow-lg transition-shadow">
              <CardHeader className="border-b">
                <div className="flex justify-between items-start">
                    <div>
                        ...
                        <CardDescription className="flex flex-col sm:flex-row sm:items-center gap-x-4 gap-y-1 mt-2">
                            ...
                            {formula.expirationDate && (() => {
                              const isExpired = new Date(formula.expirationDate) < new Date(new Date().setHours(0, 0, 0, 0));
                              return (
                                <span className={`flex items-center ${isExpired ? 'text-red-600 font-semibold' : 'text-green-600'}`}>
                                  ...
                                  {new Date(formula.expirationDate).toLocaleDateString('es-CO', { year: 'numeric', month: 'long', day: 'numeric' })}
                                </span>
                              );
                            })()}
                        </CardDescription>
                    ...
                    {(() => {
                      const isExpired = formula.expirationDate && new Date(formula.expirationDate) < new Date(new Date().setHours(0, 0, 0, 0));
              ...
              <div className="space-y-6 py-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-muted/30 rounded-lg">
                  ...
                  {selectedFormula.expirationDate && (() => {
                    const isExpired = new Date(selectedFormula.expirationDate) < new Date(new Date().setHours(0, 0, 0, 0));
                    return (
                      <div>
                  ...
                  <div>
                    ...
                    <div className="mt-1">
                      {(() => {
                        const isExpired = selectedFormula.expirationDate && new Date(selectedFormula.expirationDate) < new Date(new Date().setHours(0, 0, 0, 0));
                        return isExpired 
